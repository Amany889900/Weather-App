...
var Forecastresponse = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=bf0c5e874dc146058d0224947241112&q=${query}&days=2`);
var Forecastdata = await Forecastresponse.json();
var date = new Date(`${Currentdata.location.localtime}`);
var weekDay =  days[date.getDay()];
var secondDay = days[(date.getDay()+1)%7];
var thirdDay = days[(date.getDay()+2)%7];
...
<div class="row container m-auto">
        <div class="col col-md-6 col-lg-4 d-flex container gap-3">
          <div class="card-body bg-dark">
            <h5 class="card-title text-light border border-b-0 d-flex justify-content-between align-items-center p-2">
              ...
              <span class="">${date.getDate()}${currentMonth}</span>
            </h5>
        ...
        <div class="col col-md-6 col-lg-4 d-flex container gap-3">
          <div class="card-body bg-dark">
            <h5 class="card-title text-light text-center border border-b-0 p-2">${secondDay}</h5>
            <div class="2ndDay text-center">
              <div class="img-cont my-3"><img src="${Forecastdata.forecast.forecastday[0].day.condition.icon}" alt=""></div> <br>
               <strong class="d-inline-block my-0 text-light fs-5">${Forecastdata.forecast.forecastday[0].day.maxtemp_c}<sup>o</sup>C</strong> <br>
               <span class="d-inline-block text-light">${Forecastdata.forecast.forecastday[0].day.mintemp_c}<sup>o</sup>C</span> <br><br>
               <span class="d-inline-block mb-5 text-primary">${Forecastdata.forecast.forecastday[0].day.condition.text}</span>
            </div>
          </div>
        ...
        <div class="col col-md-6 col-lg-4 d-flex container gap-3">
          <div class="card-body bg-dark">
            <h5 class="card-title text-light text-center border border-b-0 p-2">${thirdDay}</h5>
            <div class="2ndDay text-center">
              <div class="img-cont my-3"><img src="${Forecastdata.forecast.forecastday[1].day.condition.icon}" alt=""></div> <br>
               <strong class="d-inline-block my-0 text-light fs-5">${Forecastdata.forecast.forecastday[1].day.maxtemp_c}<sup>o</sup>C</strong> <br>
               <span class="d-inline-block text-light">${Forecastdata.forecast.forecastday[1].day.mintemp_c}<sup>o</sup>C</span> <br><br>
               <span class="d-inline-block mb-5 text-primary">${Forecastdata.forecast.forecastday[1].day.condition.text}</span>
            </div>
          </div>
...
async function search(searchQuery){
  ...
  var searchResponse = await fetch(` http://api.weatherapi.com/v1/search.json?key=bf0c5e874dc146058d0224947241112&q=${searchQuery}`);
  var searchData = await searchResponse.json();

  var city = searchData[0].name;
